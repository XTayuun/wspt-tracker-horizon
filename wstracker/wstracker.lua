...
local last_equipped_id = 0;

-- CORRECTED SKILLCHAIN BONUS MAP
-- Hex to Decimal translations mapped correctly to trial points
local sc_bonus_map = {
    [288] = 4, [289] = 4, -- 0x120, 0x121: Light, Darkness
    [290] = 2, [291] = 2, [292] = 2, [293] = 2, -- 0x122-0x125: Lv2 (Grav, Frag, Dist, Fusion)
    [294] = 1, [295] = 1, [296] = 1, [297] = 1, [298] = 1, [299] = 1, [300] = 1, [301] = 1, -- 0x126-0x12D: Lv1s
    [302] = 4, [303] = 4  -- 0x12E, 0x12F: Radiance, Umbra
};
...
local function CheckWeaponMode()
    ...
    local equipment = inventory:GetEquippedItem(0); 
    ...
    if (e.id == 0x28) then
        ...
        if (action.UserId == my_id) then
            ...
            if first_target and first_target.Actions[1] then
                ...
                -- Ignore misses/evades
                if (act.Reaction == 0x01 or act.Reaction == 0x09) then return end 

                -- HORIZON XI FIX: Filters out Job Abilities (Steal/Mug) disguised as Category 3.
                -- 185 = Standard WS Damage. 187 = HP Drain WS.
                if (act.Message ~= 185 and act.Message ~= 187) then return end
